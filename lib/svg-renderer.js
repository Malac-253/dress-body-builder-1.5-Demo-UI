...
function renderGraphicalPartSVG(containerSelector, partData, colorData, fallbackName) {
  ...
  // After shapes are drawn, auto-zoom to bounding box
  requestAnimationFrame(() => {
    autoZoomToContent(svg, mainLayer);
  });
 ...
 */
function autoZoomToContent(svg, layer) {
  const box = layer.node().getBBox();
  if (box.width === 0 || box.height === 0) {
    console.warn("autoZoomToContent: no bounding box to zoom.");
    return;
  }

  // Our <svg> has a 700x700 viewBox
  const svgSize = 700; 
  // optional margin 
  const marginFactor = 0.1; 

  // scale so bounding box fits with margin
  const scale = Math.min(
    (svgSize * (1 - marginFactor)) / box.width,
    (svgSize * (1 - marginFactor)) / box.height
  );

  // center bounding box in the viewBox
  ...
  // forcibly set the transform on our existing zoom
